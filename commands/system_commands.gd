...
func restart_editor(params: Dictionary) -> Dictionary:
	...
	#
	# SEE-1134 Q2: release the WS listener (plugin.release_ws_server ->
	# websocket_server.stop_server) BEFORE restart_editor quits. The relaunched
	# editor rebinds the port at startup; freeing the socket here shrinks the
	# window where the old process still holds it, avoiding "address already in
	# use" in the new process.
	var tree := Engine.get_main_loop() as SceneTree
	if tree:
		tree.create_timer(RESTART_ACK_GRACE_SEC).timeout.connect(
			func() -> void:
				if _plugin and _plugin.has_method("release_ws_server"):
					_plugin.release_ws_server()

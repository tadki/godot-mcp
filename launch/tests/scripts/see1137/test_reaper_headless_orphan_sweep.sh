...
OUT="$(KOL_REAP_HEADLESS_GRACE_M=0 "$REAPER" --root "$TMP" --dry-run 2>&1 || true)"
...
KOL_REAP_HEADLESS_GRACE_M=30 "$REAPER" --root "$TMP" >/dev/null 2>&1 || true
...
KOL_REAP_HEADLESS_GRACE_M=0 "$REAPER" --root "$TMP" >/dev/null 2>&1 || true
sleep 0.3
if kill -0 "$FRESH_PID" 2>/dev/null; then
